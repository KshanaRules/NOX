...
def prueba():
    print ("Prueba de clase funciones")
        
def ordenarArchivos(archivos):
    for archivo in  archivos:
        cortaArchivo(archivo)
        
    lista = [int(a) for a in arr] 
    lista.sort()
    return lista
        
def cortaArchivo(archivo):
    d = archivo.split('Group')
    if (len(d)>1):
        d = d[1].split('.')
        arr.append(d[0])
    
def mostrarArchivos(lista):
    arr.clear()
    for arch in lista:
        arr.append("Group"+str(arch)+".xlsx")
    return arr

def leerArchivos(especiesFile):  
    especies = pd.read_excel("especies.xlsx")
    archivos = os.listdir(especiesFile)
    lista = ordenarArchivos(archivos)
    NumEspecies = len(lista)
    ...
    for file in lista:
        print("2")
        ArchivoCSV = pd.read_excel("DatosXLS/"+arr[ciclo])
