...
class App(tk.Tk):
    def __init__(self):
        ...
        self.btn_archivo  = tk.Button(opciones, text="Cargar datos", command=self.cargaDatos)
        self.btn_archivo.pack(padx=11, pady=11)
 
        self.btn_archivo2  = tk.Button(opciones, text="Procesar", state="disabled", command=self.procesar)
        ...
        self.btn_exportar = tk.Button(opciones, text="Mostrar gráficas", state="disabled", command=self.muestraGraficas)
        ...
        self.btn_siguiente = tk.Button(self.seccionGraf, text=">",  state="disabled", command=self.siguienteGraf)
        self.btn_siguiente.place(x=500, y=400)  
        print(os.getcwd())
        
    def procesar(self):
        datos.leerArchivos(self.dirFiles)
        
        
    def cargaDatos(self):
        filename = f.askdirectory(title="Seleccionar carpeta", initialdir=self.dir)
        if filename:
            print(filename)
            self.dirFiles = filename
            self.btn_archivo2['state'] = 'normal'

            """xls = pd.ExcelFile(filename)
            NumEspecies = len(xls.sheet_names)
            datos.leerArchivo(filename,NumEspecies, self.especiesFile)
            """
